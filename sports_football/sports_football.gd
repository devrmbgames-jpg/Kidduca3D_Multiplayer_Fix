...
func start_game() -> void :
	...
	Singletones.get_Global().lock_jump = true
...
func _play_dialog(key_text: String) -> void :
	_audio_dialog.stream = ResourceLoader.load(Singletones.get_LocaleSounds().get_sound_path(key_text), "", GlobalSetupsConsts.NO_CACHED)
	_audio_dialog.play()
...
func _init_enemys_and_friends() -> void :
	...
	var random_clothes : Array = _get_random_clothes()
	var player_pkg_path : String = Singletones.get_Global().player_character.get_character_pkg_path()
	if player_pkg_path != "":
		_character_friend = _characters.get_character_from_path_player(player_pkg_path)
		characters_list.erase(_character_friend)
		var characters_models_list_friend : Array = _characters.get_characters_models_list_one_type(_character_friend)
		characters_models_list_friend.shuffle()
		for friend in _friends.get_children():
			friend.change_skin(characters_models_list_friend[0])
			if _character_friend == _characters.CRAZY:
				friend.is_crazy = true
			friend.update_reward_items(random_clothes)
		_goalkeep_friend.change_skin(characters_models_list_friend[0])
		if _character_friend == _characters.CRAZY:
			_goalkeep_friend.is_crazy = true
		_goalkeep_friend.update_reward_items(random_clothes)
	
	characters_list.shuffle()
	_character_enemy = characters_list[0]
	var characters_models_list_enemy : Array = _characters.get_characters_models_list_one_type(_character_enemy)
	characters_models_list_enemy.shuffle()
	
	random_clothes = _get_random_clothes()
	for enemy in _enemys.get_children():
		enemy.change_skin(characters_models_list_enemy[0])
		if _character_enemy == _characters.CRAZY:
				enemy.is_crazy = true
		enemy.update_reward_items(random_clothes)
	_goalkeep_enemy.change_skin(characters_models_list_enemy[0])
	if _character_enemy == _characters.CRAZY:
		_goalkeep_enemy.is_crazy = true
	_goalkeep_enemy.update_reward_items(random_clothes)

func _get_random_clothes() -> Array :
	var clothes := [
...
func _init_player() -> void :
	Singletones.get_Global().player_character.global_transform.origin = _pos_character_start.global_transform.origin
	Singletones.get_Global().player_character.direction = Vector3.FORWARD
	...
	Singletones.get_Global().player_character.move_force = false
	Singletones.get_Global().player_character.visible = true
...
func _input_player() -> void :
	var pressed_active := Input.is_action_just_pressed("active") \
		or _is_click_right_stick \
		or Input.is_action_just_pressed("TV_PRESS")
	if pressed_active :
...
func _great(is_friend: bool) -> void :
	Haptic.play_success()
	Singletones.get_Global().paywall()
